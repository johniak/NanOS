...
- [x] **Step 1:** verified the vendored tree is complete — 276 objects (from the v6.12 i915 Makefile with the CONFIG choices below), ALL with a `.c` present. `scripts/i915-objs.txt` = the reproducible list; `scripts/build-i915.sh` + `make i915-probe` compile each and print `clean/total` + the first-error cluster map. CONFIG block added to `linuxkpi/autoconf.h`. **Baseline: 0/276** — every file blocked on a missing shim header (45 unique; `pm_qos.h` gates 145, `hashtable.h` 28, `sched/clock.h` 11, …).
- [~] **Step 2..N (in progress):** iterate clusters; commit per cluster (git history IS the campaign log).
  - **Cluster 1 DONE (missing-header tier):** created ~56 shim headers (pm_qos, hashtable, ref_tracker, io_mapping-adjacent, runtime-PM, mmzone, hypervisor, perf_event/pmu type-only, refcount, iopoll, unaligned, io-64-nonatomic, tasklet/seqcount routing, + display/acpi/gpio/pinctrl/mfd/media guards) + ~18 fixes to EXISTING shim headers (interrupt/timer/sched/kernel/atomic/math64/bitops/lockdep/jiffies/i2c/scatterlist/workqueue/swap + compat.h cpu_relax + kpi_kthread flush_delayed_work). Key routing fixes: i2c.h→workqueue.h (drm_dp), timer.h→seqlock.h (seqcount_t), sched.h→timer.h (timer_pending), and the timer_pending-inside-`extern "C"` bug. **0→7/276 clean; entire missing-header tier resolved.** No regression: virtio_gpu.nkext relinks + smoke-virtio-gpu/kpi-wq/kpi-irq green, test64 863/863.
  - **NEXT (Cluster 2):** the incomplete-type tier — `i915_request.h`/`i915_scheduler_types.h` embed types (tasklet etc) reached only transitively; same include-graph-routing pattern as Cluster 1. Then implicit-decls, then the ~210-object link phase.
