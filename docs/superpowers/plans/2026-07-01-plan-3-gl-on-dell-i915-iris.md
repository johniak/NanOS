...
- [ ] **Step 2:** Implement; convert `virtio_transport.c` to register the MSI and call the driver's configured vq callbacks (find how the driver's `vp_interrupt`-equivalent is currently pumped — the poll hook `lkpi_fence_poll_hook` — and trigger the same function from the MSI).
...
- [ ] **Step 4:** `make verify64` — ALL existing gates green (especially smoke-virtio-gpu, smoke-vt, SMP gates: MSI handler concurrency vs fine-grained locks — if the fence lock discipline trips, fix the KPI spinlock shims to be REAL spinlocks now (SMP kernel!), not boot-time no-ops; that hardening belongs to this task).
...
**Interfaces:**
- Produces: `kthread_run/kthread_stop/kthread_should_stop`; `alloc_workqueue` returns a queue backed by ONE kthread each (i915 creates few; cap 16 with fail-loud), `queue_work/queue_delayed_work` (delayed via existing kpi timer wheel), `flush_workqueue/flush_work/cancel_*` with real semantics (completion-based); `schedule_work` migrates from inline to the system workqueue thread. `request_threaded_irq`'s thread_fn moves onto a dedicated irq thread (fixing Task 2's interim).
...
**Files:**
- Modify: `scripts/vendor-linux.sh` (add `drivers/gpu/drm/i915`, `include/drm/i915_*`, `include/uapi/drm/i915_drm.h`); Create: `scripts/build-i915.sh`; Modify: `Makefile` (`I915_OBJS`, `i915.nkext` linking `kext/i915` glue + I915 objs + shared DRM core/lib + LINUXKPI objs), `linuxkpi/autoconf.h` (the `CONFIG_DRM_I915_*` block)
...
- [ ] **Step 1:** vendor + `scripts/build-i915.sh` printing `clean/total` objects; commit the vendoring separately (`vendor: linux-6.12 i915 (unmodified)`).
- [ ] **Step 2..N:** iterate clusters; commit per cluster (the git history IS the campaign log, as with drm core).
- [ ] **Final step:** `i915.nkext` links; QEMU boots with it PRESENT but idle (no 8086 display device → probe never runs): `make verify64` green with the kext on the image.
...
- Modify: boot-param plumbing: `nanos.i915=1` (find how existing boot params parse: `grep -rn "cmdline\|bootparam" kernel/ | head`) — default 0 on real HW, forced 0 when a virtio-gpu already bound the display.
