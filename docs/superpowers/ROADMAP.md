...
| **later** | **`.ndl` (Nano Dynamic Library)** — full Windows-style model: libnanos becomes a separate `.ndl` the kernel loads into the process (ring 3), program's IAT bound to it; the `.ndl` issues `int 0x80`. (Today libnanos is statically compiled into the program and traps directly — no IAT.) Also: `.nkext` kernel modules; shell (keyboard input buffer + blocking read + `ls`/`cat` as `.nxe`); ext write support. | ⬜ TODO |
...
_Last updated: end of stage 2 (init runs in ring 3, isolated). Next: stage 3 (scheduler)._
