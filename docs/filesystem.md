...
1. **The VFS namespace** — the unified path tree a running program sees (`/`, `/disks`,
   `/dev`, `/proc`, `/tmp`, …). It is assembled at boot from several filesystem drivers
...
├── disks/
│   └── main/         ext2/ext4 (auto-detected) — THE physical disk image, partition at
│                     LBA 2048. This is where everything persistent lives (see §2).
├── dev/
...
└── tmp/              RamFs — a writable in-memory tmpfs. The ONLY writable place (the disk
                      driver is read-only). Cleared on reboot. Programs put transient files
                      here (e.g. Doom's config + savegames).
```
...
**Writability:** only `/tmp` (RamFs) is writable. The ext driver (`fs/ExtFilesystem.*`) is
**read-only** and `/` + `/proc` are generated, not stored. A program that needs to write
uses `/tmp`.
...
The ext reader (`fs/ExtFilesystem.h`) resolves both kinds of link during path lookup:
...
## 4. The storage stack (how a path read reaches the disk)

```
VFS (fs/Vfs)              path routing: longest-prefix mountpoint -> a FileSystem driver
  ├── SynthFs             generated trees: / and /proc and /dev nodes
  ├── RamFs               in-memory read/write tmpfs (/tmp)
  └── ExtFilesystem       ext2/ext4 read-only core (superblock, inodes, dirs, symlinks)
        ├── Ext2Filesystem  resolveBlock = direct blocks
        └── Ext4Filesystem  resolveBlock = extent tree

BlockDevice (drivers/BlockDevice.h)   HAL: readSectors/writeSectors/sectorSize
  ├── AtaBlockDevice     real disk (ATA PIO), one command per sector
  └── RamBlockDevice     in-memory buffer (and the test fixtures)

DeviceManager            runtime registry of block devices
```

A read of `/disks/main/apps/doom/doom.nxe` → VFS matches the `/disks/main` mount → the ext
driver resolves the path (following the symlink if it came in via `/bin`) → `resolveBlock`
maps file offsets to disk blocks → `AtaBlockDevice` issues the ATA reads.
