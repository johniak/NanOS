...
# Wait for the actual shell prompt (NOT "starting shell/services", which prints before init runs).
# OVMF + the dropbear ed25519 keygen make first boot slower than BIOS, so allow up to 60s.
for i in $(seq 1 60); do grep -q "bash-5" "$SER" 2>/dev/null && break; sleep 1; done
sleep 2
