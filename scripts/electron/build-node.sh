...
# _POSIX_SEM_VALUE_MAX: picolibc's <limits.h> omits this POSIX constant, but libstdc++'s C++20
# <semaphore> uses it in a static_assert for counting_semaphore's max. Supply the Linux value.
NANOS_CFLAGS="$NANOS_CFLAGS -D_POSIX_SEM_VALUE_MAX=32767"
...
# gaps in one place rather than per call site.
NANOS_CFLAGS="$NANOS_CFLAGS -D_GNU_SOURCE"
