...
parsetime(char *str)
{
	...
	/* NanOS local patch: NanOS has no timezone database (localtime() == UTC) and picolibc's
	 * struct tm has no tm_gmtoff/tm_zone, so a Zulu (Z) timestamp needs no gmtoff adjustment. */
	(void) zulu;
