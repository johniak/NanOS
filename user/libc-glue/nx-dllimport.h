...
/* The IAT slots (`__imp_*`) are plain C symbols in libc.ndl / dllimport.S. When a C++ TU pulls this
 * in (V8, Chromium), the system headers expand `environ`/`_ctype_b`/... to those same slots INSIDE
 * their `__BEGIN_DECLS` extern "C" block — so our own slot declarations must also be extern "C", or
 * the two collide ("conflicting declaration ... with 'C' linkage"). Pure-C ports never saw this. */
#ifdef __cplusplus
#define NX_IMP extern "C"
#else
#define NX_IMP extern
#endif
...
NX_IMP const char (*__imp__ctype_b)[];
...
NX_IMP FILE **__imp_stdin;
NX_IMP FILE **__imp_stdout;
NX_IMP FILE **__imp_stderr;
NX_IMP char ***__imp_environ;
