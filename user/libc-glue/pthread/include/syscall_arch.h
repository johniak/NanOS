/*
 * syscall_arch.h — NanOS arch-aware syscall thunks for the vendored musl pthread core.
 *
 *   - x86_64 : the `syscall` instruction (mirrors musl arch/x86_64/syscall_arch.h). nr in
 *              %rax, args in %rdi/%rsi/%rdx/%r10/%r8/%r9 — note arg4 is %r10, NOT %rcx,
 *              because the `syscall` instruction clobbers %rcx (return RIP) and %r11
 *              (rflags); result in %rax; clobbers "rcx","r11","memory". 64-bit args, so
 *              __SYSCALL_LL_E/_O are the identity.
 *   - i386   : `int $0x80` (or the vDSO entry via %gs:16), args in ebx/ecx/edx/esi/edi/ebp,
 *              with the ebx/edx swap dance for PIC — kept verbatim below.
 */
