...
/* NanOS addition: upstream musl declares these in its src/include/{time,pthread}.h build
 * overlays (which add the hidden internal prototypes shadowing the public headers). We don't
 * vendor those overlays, so the two prototypes __timedwait.c needs are declared here instead.
 * Definitions live in user/libc-glue/pthread/nanos_glue.c. */
