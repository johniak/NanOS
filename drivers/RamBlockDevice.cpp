...
static bool inRange(uint64_t lba, unsigned count, unsigned byteSize) {
...
int RamBlockDevice::readSectors(uint64_t lba, unsigned count, void* buf) {
...
int RamBlockDevice::writeSectors(uint64_t lba, unsigned count, const void* buf) {
	if (!inRange(lba, count, byteSize))
		return -1;
